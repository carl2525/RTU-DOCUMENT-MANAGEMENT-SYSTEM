...
<div class="container-fluid">
    ...
    <a href="addfile.php"><button type="button" class="btn btn-success waves-effect waves-light"><svg class="" width="20" height="20"><use xlink:href="#view"></use></svg>View File Document</button></a>
    <hr>
    </div>
  </div>
...
$query=mysqli_query($conn,"SELECT * FROM `upload_files` WHERE `NAME` = '$filename' AND `ARCHIVE` = '0'")or die(mysqli_error($conn));
  ...
  icon: 'error',
  title: 'Error Uploading File!!',
  text:'The Files is Already on the Database',
  confirmButtonColor:'#00396D',
   confirmButtonText: 'Okay',
   timer:3000
        ...
        date_default_timezone_set("asia/manila");
         ...
         if(!is_dir("../uploads/".$foldername)) mkdir("../uploads/".$foldername);
          if(move_uploaded_file($file,"../uploads/".$foldername."/".$filename)){
            $sql = "INSERT INTO upload_files (NAME, DEPARTMENT, SIZE, DESCRIPTION,SENDTO, DOWNLOAD, TIMERS, ADMIN_STATUS, EMAIL, YEAR, SEEN_STATUS) VALUES ('$filename','$foldername', '$size','$description','$sendto', 0, '$time', 'Admin', '$user', '$year',0)";
...
if (($sendto != "Private") && ($sendto != "All")){
  ...
  $mail= new PHPMailer(true);
    ...
    //Recipients
    $mail->setFrom('cergorobao@rtu.edu.ph', 'RTU|DMS Share a New File');
    $mail->addAddress($sendto);     //Add a recipient        //Name is optional
    $mail->addReplyTo('no-reply@cergorobaortu.edu.ph', 'No Reply');

    $filename1 = array('$filename');

    $url="https://".$_SERVER["HTTP_HOST"].dirname($_SERVER["PHP_SELF"]). "/index.php";

    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject = 'Someone Share a File With You at RTU|DMS';
    $mail->Body    = "<h1>Someone Share a File With You at RTU|DMS</h1>
                      <h3>Hi $sendto,</h3>
                      $user Sent You a File<br>
                      <h3>The Filename is: </h3>
                      $filename1<br>

                      Please Checkout this File and See What's Inside at RTU|DMS<br>

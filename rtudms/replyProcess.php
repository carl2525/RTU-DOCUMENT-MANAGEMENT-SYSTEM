...
require_once("connection.php");
 ...
 if(isset($_POST['edit'])){
    ...
    $time = date("M-d h:i A",strtotime("+0 HOURS"));
    $year = date("Y",strtotime("+0 HOURS"));

       $q = mysqli_query($conn,"SELECT * FROM `upload_files` WHERE id = '$_REQUEST[id]'") or die (mysqli_error($conn));
        ...
        $rs1 = mysqli_fetch_array($q);
               ...
               $sendto = $rs1['SENDTO'];
               $download = $rs1['DOWNLOAD'];
               $status = $rs1['ADMIN_STATUS'];
               $user = $rs1['EMAIL'];
               $seen = $rs1['SEEN_STATUS'];
               $archive=$rs1['ARCHIVE'];
                $sentby = $rs1['SENTBY'];
               $designation=$rs1['DESIGNATION'];

    $conn->query ("INSERT INTO send_history (controlNumber,NAME, DEPARTMENT, SIZE, DESCRIPTION, SENTBY,SENDTO, DOWNLOAD, TIMERS, DESIGNATION,ADMIN_STATUS, EMAIL, YEAR, SEEN_STATUS, ARCHIVE) VALUES ('$controlno','$fname1','$dept', '$size','$description','$sentby','$sendto', '$download', '$time', '$designation','$status', '$user', '$year','$seen', '$archive')")or die(mysqli_error());

    $description = mysqli_real_escape_string($conn,$_POST['description']);
    $sendTo = mysqli_real_escape_string($conn,$_POST['sendTo']);

    $myfileCount = $_FILES['myfile']['name'];
    $myfileCount1 = $myfileCount;

if(is_null($myfileCount1)){
     $conn->query("UPDATE `upload_files` SET `DESCRIPTION` = '$description', `SENDTO` = '$sendTo',`SENTBY` = '$idemail' WHERE `id` = '$_REQUEST[id]'") or die(mysqli_error());
  ...
  confirmButtonColor:'#00396D',
   ...
   timer:3000
    ...
    } else if ($_FILES['myfile']['size'] > 52428800) { // file shouldn't be larger than 1Megabyte
      echo "<script type='text/javascript'>
      document.querySelector('body').style.overflow = 'hidden';
      Swal.fire({
  icon: 'error',
  ...
  confirmButtonColor:'#00396D',
   ...
   timer:3000
          ...
          if(move_uploaded_file($file,"../uploads/".$foldername."/".$filename)){
               $sql = "UPDATE `upload_files` SET `NAME` = '$filename', `SIZE`  = '$size', `DESCRIPTION`  = '$description',`SENDTO`  = '$sendTo', `SENTBY` = '$idemail' WHERE `id` = '$_REQUEST[id]'";
            ...
            $insertInNotification = "INSERT INTO notifcation_table (id, controlNumber, EMAIL, SENDTO, ACTION, FILENAME, TIMERS, SEEN_STATUS, LINK) 
              VALUES (0, '$controlActionID', '$idemail', '$sendTo', 'Reply to Your Shared File Named', '$filename', '$time', 0, 'sharedwithme')";

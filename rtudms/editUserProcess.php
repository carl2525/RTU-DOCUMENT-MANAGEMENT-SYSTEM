...
<html lang="en">
  <head>
    ...
    <title>RTU|DMS View User Account</title>
    ...
    <li>
      ...
      <li>
       ...
       <li>
        ...
        <li>
        <a href="user_log.php" class="nav-link dropdown-item text-white active">
          <svg class="bi pe-none me-2" width="16" height="16"><use xlink:href="#history"/></svg>
           - User Login History
        </a></li>
      </ul>
   ...
   <div class="container-fluid mt-5">
      ...
      <div class="card mb-4 wow fadeIn">
        ...
        <h4 class="mb-2 mb-sm-0 pt-1 fw-bold">
            ...
            <span>|</span>
            <span style="color: #E0A100;">USER ACCOUNT</span>
          </h4>
<!-- 
     ...
     <?php
         require_once("connection.php");
          ...
          ?>       
           ...
           <tr>
             <td width='5%' class="fw-bold"><?php echo $id; ?></td>
               <td width='12%'><?php echo  $fname; ?></td>
               <td width='20%'><?php echo $admin; ?></td>
               <td width='20%'><?php echo $pass; ?></td>
               <td width='8%'><a href="edit_user.php?id=<?php echo $id;?>">
<svg class="" width="20" height="20"><use xlink:href="#edit"/></svg></a> | <a onclick="yesNO()"><svg class="" width="20" height="20" style="color:red; cursor: hand;"><use xlink:href="#trash"/></svg>
            <script>
              function yesNO(){
                Swal.fire({
                    title: 'Are you sure?',
                    ...
                    if (result.isConfirmed) {
                      window.location.href='delete_user.php?id=<?php echo htmlentities($rs['id']); ?>'
                    }
                  })
              }
            </script>
...
if (val === "User") {
 ...
 if(isset($_POST['edit2'])){
  ...
  //  $password=$salt.$pass;
    ...
    $q_checkadmin = $conn->query("SELECT * FROM `login_user` WHERE `email_address` = '$email_address' AND `user_password` ='$user_password'AND `name`='$user_name'") or die(mysqli_error());
    $q_checkpassword = $conn->query("SELECT * FROM `login_user` WHERE `user_password` ='$user_password'") or die(mysqli_error());
    $q_checkname = $conn->query("SELECT * FROM `login_user` WHERE `name` ='$user_name'") or die(mysqli_error());
    $q_checkemail1 = $conn->query("SELECT * FROM `login_user`WHERE `email_address` = '$email_address'") or die(mysqli_error());
...
document.querySelector('body').style.overflow = 'hidden';
 Swal.fire({
  icon: 'error',
  title: 'Error',
  text:'You Didnt Change any Information on the User Account',
  confirmButtonColor:'#00396D',
   ...
   timer:3000
       ...
       $user_password1 = password_hash($_POST['user_password'], PASSWORD_DEFAULT, array('cost' => 12));
      $conn->query("UPDATE `login_user` SET `name` = '$user_name', `email_address` = '$email_address', `user_password` = '$user_password1' WHERE `id` = '$_REQUEST[id]'") or die(mysqli_error());
       echo "<script type='text/javascript'>
       document.querySelector('body').style.overflow = 'hidden';
       Swal.fire({
  icon: 'success',
  title: 'Success!!',
  text:'Information on the User Account Has Been Successfully Updated',
  confirmButtonColor:'#00396D',
   confirmButtonText: 'Okay',
   timer:3000
}).then(function(){
        ...
        $conn->query("UPDATE `login_user` SET `name` = '$user_name', `email_address` = '$email_address' WHERE `id` = '$_REQUEST[id]'") or die(mysqli_error());
       echo "<script type='text/javascript'>
       document.querySelector('body').style.overflow = 'hidden';
       Swal.fire({
  icon: 'success',

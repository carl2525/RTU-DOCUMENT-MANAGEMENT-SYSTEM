      ...
      <div class="card mb-4 wow fadeIn">
        ...
        <div class="card-body d-sm-flex justify-content-between shadow bg-light rounded">
          ...
          <h4 class="mb-2 mb-sm-0 pt-1 fw-bold">
            ...
            <span>|</span>
            <span style="color: #E0A100;">CREATE A REQUEST</span>
          </h4>
<!-- 
...
<div class="container-fluid">
    ...
    <a href="addrequests.php"><button type="button" class="btn btn-success waves-effect waves-light"><svg class="" width="20" height="20"><use xlink:href="#view"></use></svg>View All Requests</button></a>

    <a href="ownrequests.php"><button type="button" class="btn btn-success waves-effect waves-light"><svg class="" width="20" height="20"><use xlink:href="#view"></use></svg>View My Requests</button></a>
    <hr>
    </div>
  </div>
...
<div class=" card rounded-5 border-bottom border-top border-5 mb-5">
<h5 class="card-header info-color rounded-5  white-text text-center py-4">
  <strong>CREATE REQUEST FORM</strong>
  </h5>
  <div class=" card-body rounded-5 px-lg-6 pt-0">
    <div class="container">
      <div class="row"><br><br>
        <form action="requestsprocess.php" id="add_form" method="post" enctype="multipart/form-data">
...
<center>
<div class="col-md-10 mt-2">
  <textarea class="form-control rounded-4" id="exampleFormControlTextarea1" name="description" rows="4" placeholder="Enter Description Message" style="resize: none;" required></textarea>
</div>
</center>
<div class="col-md-5 mt-2">
        <input type="text" maxlength="8" onkeypress="{return isNumberKey(event)}" inputmode="numeric" class="form-control rounded-4" id="email" placeholder="Req Control# 8 Digits Only" name="requestid">
...
<div class="col-md-5 mt-2">
  <select class="form-select fw-bold rounded-4" id="customOffice" name="department" aria-label="Default select example" required >
    <option selected disabled value="">Choose Department/Office</option>
 <?php
         require_once("connection.php");
        ...
        while($file=mysqli_fetch_assoc($result)){
         ...
         $offices = $file['OFFICES'];
          ...
          ?>    
    <option value="<?php echo $offices; ?>"><?php echo $offices; ?></option>  <?php  } ?>
          ...
          <button type="submit" class="btn fw-bold btn-rounded btn-block mt-3 mb-3 waves-effect z-depth-0 rounded-4" name="save" id="addnew"style ="background:#E0A100;" >Create Request</button>
...
<div class="footerstyleUploadRequest">
...
$query=mysqli_query($conn,"SELECT * FROM `requests`")or die(mysqli_error($conn));
        ...
        date_default_timezone_set("asia/manila");
         ...
         $year = date("Y",strtotime("+0 HOURS"));
            ...
            $sql = "INSERT INTO requests (requestid,department,request,sendto,timers,email,status, seen_status) VALUES ('$reqid','$foldername','$description','$sendto','$time','$user','Requesting...','0')";

            if (mysqli_query($conn, $sql)) {

              $controlActionID= "REQUEST ID#: ".$reqid;

             $insertInNotification = "INSERT INTO notifcation_table (id, controlNumber, EMAIL, SENDTO, ACTION, FILENAME, TIMERS, SEEN_STATUS, LINK) 
              VALUES (0, '$controlActionID', '$user', '$sendto', 'Sent you a Request with Description of', '$description', '$time', 0, 'requested')";
              ...
              Swal.fire({
                icon: 'success',
                title: 'Request Sent!!',
                text:'Your Request has been successfully sent to $sendto.',
                confirmButtonColor:'#00396D',
                 confirmButtonText: 'Okay',
                 timer:3000
              }).then(function(){
                        window.location='add_requests.php'
                        })

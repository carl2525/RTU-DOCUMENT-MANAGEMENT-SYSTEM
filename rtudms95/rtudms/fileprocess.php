...
<?php 
 ...
 $row = mysqli_fetch_array($r);
  ...
  $admin_department = $row['designation'];

 $code = mysqli_query($conn,"SELECT * FROM department_office where offices = '$admin_department'");
 $coderow = mysqli_fetch_array($code);
 $codeprint=$coderow['id'];
...
if (isset($_POST['save'])) { // if save button on the form is clicked
    ...
    $filename = 'RTU'.$codeprint.' - '.$_FILES['myfile']['name'][$i];
         ...
         if(!is_dir("../uploads/".$foldername)) mkdir("../uploads/".$foldername);
          if(move_uploaded_file($file,"../uploads/".$foldername."/".$newfilename)){
            ...
            if (mysqli_query($conn, $sql)) {
              ...
              $timeupload = date("h:i:s A",strtotime("+0 HOURS"));
              ...
              $insertQueryUpload = "INSERT INTO upload_log (NAME, DEPARTMENT, SIZE, UPLOAD, MONTH, TIME, YEAR, ACC_STATUS, EMAIL) 
              VALUES ('$newfilename', '$foldername', '$size', '1', '$month', '$timeupload', '$year', 'Admin', '$user')";

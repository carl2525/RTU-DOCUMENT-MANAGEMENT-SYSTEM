...
<div class="">
<div class="" <?php if(isset($_POST['submit'])){ echo "style='display:block'"; } ?>>
...
<div class="form-floating mb-4 mt-0" >
    ...
    <div class="ms-2">
        <button type="submit" id="submitbtn" onclick="goback()" name="submit" class="mt-0 mb-3 btn col-12 text-dark btn-floating rounded-3 font-weight-bold" style="background:#E0A100; font-weight:bold; " textContent="">Request Password Reset <i class="fa fa-arrow-right"></i></button>
...
if(isset($_POST["email"])){
  ...
  $email = $_POST['email'];
    ...
    $queryAdmin = "SELECT * FROM admin_login WHERE admin_user ='$email'";
                $resultAdmin =  mysqli_query($conn, $queryAdmin) or die ("Error in query: $queryAdmin. ".mysqli_error($conn)); 
                $rowAdmin = mysqli_fetch_assoc($resultAdmin);
                $name=$rowAdmin['name'];
                $email = $rowAdmin['admin_user'];


    if($rowAdmin['admin_user']>0){
    ...
    //Recipients
    $mail->setFrom('cergorobao@rtu.edu.ph', 'RTU|DMS Password Reset');
   ...
   //Optional name
    ...
    $mail->Body    = "<h1>You Requested a Password Reset</h1>
                      <h3>Hi $name,</h3>
                      A Password reset was requested for your account at RTU|DMS.<br>
  ...
  text:'Please Check Your Email in Order to Confirm Your Forget Password!!',
...
exit();
  ...
  text:'Please Check Your Inputted Email Address Properly!!',
  confirmButtonColor:'#00396D',
   confirmButtonText: 'Okay',
   timer:3000

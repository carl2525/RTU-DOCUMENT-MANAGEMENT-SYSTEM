...
<div class="row d-md-block w-100 ms-0">
    ...
    <li>
        <a href="#" class="nav-link text-white" data-bs-toggle="modal" data-bs-target="#modalRegisterForm">
          <svg class="bi pe-none me-2" width="16" height="16"><use xlink:href="#users"/></svg>Add Employee Account
        </a>
      </li>
      <li>
   ...
   <div class="container-fluid mt-5 rounded-4 ">
      ...
      <div class="card mb-4 wow fadeIn rounded-4 ">
        ...
        <h4 class="mb-2 mb-sm-0 pt-1 fw-bold rounded-4 ">
            ...
            <span>|</span>
            <span style="color: #E0A100;">ALL OLD DOCUMENTS</span>
          </h4>
<!-- 
...
<div class="d-flex">
                    ...
                    <form action="pastFiles.php" method="POST">
    ...
    <option name="offices" value="<?php echo $offices; ?>"><?php echo $offices; ?></option>  <?php  } ?>
                    ...
                    <div class="form-group ps-2">
                        <div class="input-group">
                            ...
                            <button class="d-inline-block fw-bold btn waves-effect waves-light rounded-4 " style="background-color: #E0A100;"><svg class="" width="20" height="20"><use xlink:href="#left"/></svg>Back to Recent Files</button>
                            </a>
               </span>
...
<table id="dtable" class = "table table-striped w-100">
    ...
    <th class="desktop">Action</th>
     ...
     if(isset($_POST['department'])){
        ...
        if($offices == "all"){
          ...
          $query="SELECT DISTINCT ID,controlNumber,NAME,DEPARTMENT,EMAIL,ADMIN_STATUS,TIMERS,YEAR,DOWNLOAD,SIZE FROM upload_files WHERE YEAR < '$currentYear' AND ARCHIVE = '0' group by ID DESC";
          $result=mysqli_query($conn,$query);
 ...
 <script>
              function downloadYes(ev){
               ...
               const swalWithBootstrapButtons = Swal.mixin({
                ...
                swalWithBootstrapButtons.fire({
                  ...
                  text: "This File is Property of RTU and Must be Use in the RTU educational purposes only.",
                  icon: 'warning',
      ...
      <?php  }
        ...
        } elseif($offices!="All" && $row['YEAR'] < $currentYear ){
        
         require_once("connection.php");

        $query="SELECT DISTINCT ID,controlNumber,NAME,DEPARTMENT,EMAIL,ADMIN_STATUS,TIMERS,YEAR,DOWNLOAD,SIZE FROM upload_files 
        WHERE (DEPARTMENT = '$offices' AND YEAR < $currentYear AND ARCHIVE = '0') GROUP BY ID DESC";
        $result=mysqli_query($conn,$query);
 ...
 <script>
    ...
    <?php  } }
         ...
         $query="SELECT DISTINCT ID,controlNumber,NAME,DEPARTMENT,SIZE,EMAIL,ADMIN_STATUS,TIMERS,YEAR,DOWNLOAD,SIZE FROM upload_files WHERE (DEPARTMENT = '$offices' AND YEAR < $currentYear AND ARCHIVE = '0') group by ID desc";
        $result=mysqli_query($conn,$query);
        while($file=mysqli_fetch_array($result)){
          $id =  $file['ID'];
         $controlid =  $file['controlNumber'];
 ...
 <script>
    ...
    <?php  }
         ...
         $query="SELECT DISTINCT ID,controlNumber,NAME,DEPARTMENT,SIZE,EMAIL,ADMIN_STATUS,TIMERS,YEAR,DOWNLOAD,SIZE FROM upload_files WHERE YEAR < $currentYear AND ARCHIVE = '0' group by ID desc";
        $result=mysqli_query($conn,$query);
        while($file=mysqli_fetch_array($result)){
          $id =  $file['ID'];
         $controlid =  $file['controlNumber'];

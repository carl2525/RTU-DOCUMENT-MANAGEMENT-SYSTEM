...
<div class="row d-md-block w-100 ms-0">
    ...
    <li>
      ...
      <li>
        <a href="add_offices.php" class="nav-link text-white active">
   ...
   <div class="container-fluid mt-5 rounded-4">
      ...
      <div class="card mb-4 wow fadeIn rounded-4">
        ...
        <h4 class="mb-2 mb-sm-0 pt-1 fw-bold rounded-4">
            ...
            <span>|</span>
            <span style="color: #E0A100;">ADD DEPARTMENT OFFICE</span>
          </h4>
<!-- 
...
<div class="container-fluid">
    ...
    <a href="#" data-bs-toggle="modal" data-bs-target="#addOffice"><button type="button" class="btn btn-success waves-effect waves-light rounded-4"><svg class="bi pe-none me-2" width="12" height="16"><use xlink:href="#building"/></svg>Add Department Offices</button></a>
    <hr>
    </div>
  </div>
  <div class="col-md-12">
...
<table id="dtable" class = "table table-striped w-100">
    ...
    <th>ACTION</th>
     ...
     <?php
        ...
        $query="SELECT DISTINCT ID,OFFICES FROM department_office ORDER by OFFICES DESC";
      ...
      <td width="30%"><?php echo  $offices; ?></td>

                 <td>
                  <a href="delete_office.php?id=<?php echo $id; ?>" class="btn btn-sm btn-outline-danger" onclick="yesNO(event)"><i class="fa fa-trash"></i>
            <script>
              function yesNO(ev){
               ...
               const swalWithBootstrapButtons = Swal.mixin({
                ...
                swalWithBootstrapButtons.fire({
                  ...
                  if (result.isConfirmed) {
                    ...
                    swalWithBootstrapButtons.fire(
                      'Cancelled!',
                      'Your Department Office is Safe and Didnt Delete.',
                      'error'
                    )
...
<label class="ms-2 mb-1"><svg class="" width="20" height="20"><use xlink:href="#building"/></svg>  Designation Office</label>
  ...
  <form action="create_office.php" method="POST" id="form">
  <div class="modal-dialog modal-dialog-centered rounded-5" role="document">
    <div class="modal-content bg-light rounded-5">
      <div class="modal-header text-center rounded-5">
        <h4 class="modal-title w-100 font-weight-bold" style="color: #10164D;"><svg class="" width="32" height="32"><use xlink:href="#building"/></svg> ADD DEPARTMENT OFFICES</h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal">
        </button>
      </div>
      <div class="modal-body">
           <div class="md-form mb-2">
            <center>
            <img src="../assets/img/office3.gif" height="300" width="400">
            </center>
        </div>
        
       <div class="form-floating mb-3">
        <input type="text" class="form-control rounded-4" id="name" placeholder="Enter Department Office" name="nameOffice">
        <label for="email"><svg class="" width="20" height="20"><use xlink:href="#building"/></svg>    Add Department Offices</label>
    </div>      
   
      <div class="modal-footer d-flex justify-content-center" id="reg" >
        <button class="btn btn-warning fw-bold rounded-4"  name="reg">CREATE DEPARTMENT OFFICE </button>
      </div>
...
require_once("connection.php");
   ...
   if(isset($_POST['reg'])){
         ...
         $offices = mysqli_real_escape_string($conn,$_POST['nameOffice']);
         

  $q_checkadmin = $conn->query("SELECT * FROM `department_office` WHERE `offices` = '$offices'") or die(mysqli_error($conn));
    ...
    if($v_checkadmin == 1){
      ...
      Swal.fire({
  icon: 'error',
  title: 'Error Creating Offices!!',
  text:'The Inputted Department Offices Name is Already Created!!',
  confirmButtonColor:'#00396D',
   confirmButtonText: 'Okay',
   timer:3000

}).then(function(){
          window.location='add_offices.php'
          })
          document.querySelector('body').style.overflow = 'visible';
         </script>";
    }else{
      $conn->query("INSERT INTO `department_office` VALUES('RTU','$offices')") or die(mysqli_error($conn));
echo "<script type='text/javascript'>
document.querySelector('body').style.overflow = 'hidden';
Swal.fire({
  icon: 'success',
  title: 'Offices Succesfully Created!!',
  text:'The Creation of Department Offices Has Been Succesfully Created!!',
  confirmButtonColor:'#00396D',
   confirmButtonText: 'Okay',
   timer:3000
}).then(function(){
